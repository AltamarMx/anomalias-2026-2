---
title: "ARMA Cheatsheet · Box-Jenkins de bolsillo"
subtitle: "Referencia rápida del laboratorio La Ventosa"
date: today
---

Resumen de los conceptos, fórmulas y decisiones del laboratorio `014b_LaVentosa.py`.
Pensado como **referencia rápida** — los ejemplos físicos y discusiones detalladas
viven en el lab.
...
## Identificación visual · ACF vs PACF
...
los pone el optimizador después por máxima verosimilitud (MLE).
:::

## Raíz unitaria y estacionariedad
...
con raíces $z_1 = 1$ (raíz unitaria) y $z_2 = 2$. Diferenciar
$\Delta y_t = y_t - y_{t-1}$ remueve la raíz unitaria y el proceso se vuelve un AR(1)
estable — equivale a un **ARIMA(1, 1, 0)** sobre la serie original.
...
::: {.callout-warning title="Necesario pero no suficiente para ARMA"}
ADF es ciego a estacionalidad determinista. Un ciclo periódico puede pasar el ADF y
aún arruinar el ARMA, o confundir al test y darle un "no estacionaria" falso.
**Siempre inspecciona ACF y climatología** antes de ajustar.
:::

## Ljung-Box · blancura de residuales
...
**Estadístico** (LB = **L**jung-**B**ox, Greta M. Ljung & George E. P. Box, 1978):
...
## Pitfalls y reglas operativas

- **ADF dice "estacionaria" ≠ se puede ARMA.** Inspecciona también ACF y
  climatología para descartar estacionalidad determinista.
- **ACF residual con picos a múltiplos de $s$** → estacionalidad pendiente. ARMA
  no la modela. Resta climatología o pasa a **SARIMA**$(p,d,q)(P,D,Q)_s$.
...
Versiones limpias de las funciones del lab:
...
## Lo que aprendimos en La Ventosa
...
5. **Backtest contra persistencia ingenua** puede mostrar que el modelo "más
   estadísticamente correcto" pierde en MAE — Box-Jenkins es necesario, no
   suficiente.
...
::: {.callout-important title="Próximo paso natural"}
**SARIMA y SARIMAX** (con exógenas como presión, gradiente térmico, índices
climáticos) — motivado por necesidad observada, no por currículum.
:::

## Lecturas

- Box, G. E. P., Jenkins, G. M., Reinsel, G. C. & Ljung, G. M. (2015). *Time
  Series Analysis: Forecasting and Control* (5th ed.). Wiley. Caps. 3–4.
- Cryer, J. D. & Chan, K.-S. (2008). *Time Series Analysis: With Applications
  in R*. Springer. Caps. 4–6.
- Hyndman, R. J. & Athanasopoulos, G. *Forecasting: Principles and Practice*.
  <https://otexts.com/fpp3/> — capítulo 9.
